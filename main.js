...
async function checkBan() {
    try {
        const response = await axios.get('https://raw.githubusercontent.com/smohamd/MOHAMED_V_999/main/ban.json');
        const data = response.data;
        
        if (data.ban === false) {
            console.log("THE ROBOT WAS STOPPED  BY THE MANUFACTURER MOHAMED AND HIS BROTHER ZINO\n\nWe are Mr. Shadi's students");
            process.exit(0); 
        }
    } catch (error) {
        console.error("404خطا تحميل ", error.message);
    }
}
checkBan();  
